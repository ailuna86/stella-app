#!/usr/bin/env python3
"""
VA / ST.ELLA Gold Full Pipeline Orchestrator v1.4.8
==================================================
...
Changes from v1.4.7 (see the master blueprint's target-architecture section
for the full rationale -- this file wires what that document specifies):

1. Evaluator now runs right after Score Contract, before Priority Engine,
   instead of after Feedback. This is a real ordering fix, not cosmetic:
   Priority Engine's own code already has a read path for Evaluator's
   strengths/semantic evidence (evaluator_payload.strengths_profile,
   layer0_5_semantic_recoverability.semantic_summary), but could not
   receive it while Evaluator ran after Priority in v1.4.7.
2. New "prior_context" stage at the very start, wrapping
   gold_session_continuity_loader_v1.py. Loads the persisted per-student
   profile (if any) so this run is not blind to prior sessions -- the
   continuity loop Premium already has via load_prior_learner_profile() /
   load_previous_directive(), which Gold's orchestrator previously had no
   equivalent of.
3. New "progress_tracker" stage right after Score Contract, wrapping
   progress_tracker_v2_scorer_feed.py. Records this essay's *adjudicated*
   score (via Score Contract's released_score, downstream of Adjudicator --
   not the raw Scorer pass) into per-student history, gated on
   progress_tracking_allowed.
4. New "persisted_profile" stage right after learner_profile, wrapping
   gold_profile_persist_v1.py. Writes the updated profile to the fixed
   per-student path prior_context will reload next run, gated on
   lie_update_allowed.
5. LRET quality gate now checks for engine version "1.12.0" (the new
   canonical LRET target) instead of "1.4.6".
...
Typical usage:
python gold_full_pipeline_orchestrator_v1_4_8.py --input submission.json --engine-config gold_engine_commands_full_v1_4_8.json --output-root gold_sessions --pretty

For development, you can copy existing artifacts instead of running engines:
python gold_full_pipeline_orchestrator_v1_4_8.py --input submission.json --copy-from-session previous_session_dir --output-root gold_sessions --pretty
"""
...
ENGINE_VERSION = "1.4.8-continuity-loop-evaluator-reorder-progress-tracker-standalone"
