...
def _extract_topic_alignment_risk(record: Dict[str, Any]) -> Dict[str, Any] | None:
    """V3 Section 3: read the Detector's cheap topic-alignment safety-net flag straight
    off the upstream record (it survives the full detector_for_scorer / evaluator_rubric_bridge
    chain as a permissive top-level pass-through -- confirmed directly against both files,
    neither strips unrecognized keys). Returns None if absent so callers can fail safe.
    """
    tar = record.get("topic_alignment_risk")
    return tar if isinstance(tar, dict) else None
...
SCORING_VERSION = "premium_unified_tier_aware_v1_4_2_topic_ceiling"

DEFAULT_CONFIG = {
    "band_thresholds": {"8": 0.88, "7": 0.74, "6": 0.60, "5": 0.45, "4": 0.30},
    # V3 Section 3: Detector safety-net hard ceiling on task_response for genuinely
    # off-topic essays, applied unconditionally (not subject to the true_fail rescue
    # heuristics below, which exist to protect against a DIFFERENT failure mode --
    # false-positive structural coverage fails on otherwise-strong essays -- and would
    # otherwise rescue exactly the off-topic-but-well-written case this ceiling exists to catch).
    "topic_alignment_risk_confidence_min": 0.55,
    "topic_alignment_risk_task_response_cap": 3,
...
def score_unified(score_input: PremiumScoreInput, profile: CanonicalMetricProfile, config: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    topic_risk = _extract_topic_alignment_risk(score_input.upstream_record or {})
    topic_ceiling = {"applied": False, "reason": None, "detector_signal": topic_risk}
    if (
        isinstance(topic_risk, dict)
        and topic_risk.get("risk_flag")
        and float(topic_risk.get("confidence", 0.0) or 0.0) >= float(cfg.get("topic_alignment_risk_confidence_min", 0.55))
    ):
        cap_value = int(cfg.get("topic_alignment_risk_task_response_cap", 3))
        current_tr = solved["final_criterion_bands"].get("task_response")
        if isinstance(current_tr, (int, float)) and current_tr > cap_value:
            solved["final_criterion_bands"]["task_response"] = cap_value
            solved["overall_band"] = _overall_half({k: int(v) for k, v in solved["final_criterion_bands"].items()})
            solved.setdefault("review_flags", []).append("topic_alignment_risk_hard_ceiling_applied")
            topic_ceiling = {
                "applied": True,
                "reason": (
                    f"Detector topic-alignment safety net flagged this essay as off-topic "
                    f"(same_topic={topic_risk.get('same_topic')!r}, confidence={topic_risk.get('confidence')}); "
                    f"task_response capped at {cap_value} regardless of the Evaluator's own relevance judgment, "
                    f"per GOLD_PIPELINE_SPEC_V3_TASK_RELEVANCE.md Section 3."
                ),
                "detector_signal": topic_risk,
                "previous_task_response_band": current_tr,
                "capped_task_response_band": cap_value,
            }
    ...
    return {
        ...
        "topic_alignment_risk_ceiling": topic_ceiling,
