#!/usr/bin/env python3
"""
VA / ST.ELLA Gold Full Pipeline Orchestrator v1.4.7
==================================================
...
Typical usage:
python gold_full_pipeline_orchestrator_v1_4_7.py --input submission.json --engine-config gold_engine_commands.json --output-root gold_sessions --pretty

For development, you can copy existing artifacts instead of running engines:
python gold_full_pipeline_orchestrator_v1_4_7.py --input submission.json --copy-from-session previous_session_dir --output-root gold_sessions --pretty
"""
...
ENGINE_VERSION = "1.4.7-production-readiness-quality-gated-orchestration-only-standalone"
...
ARTIFACTS: Dict[str, str] = {
    ...
    "priority_input": "03a_priority_input_v1_4_7.json",
...
def _scorer_evidence_features(obj: Any) -> Dict[str, float]:
    ...
    # Some scorer versions expose the evidence ledger directly.
...
def validate_quality_gates(paths: Dict[str, Path]) -> List[Dict[str, str]]:
    ...
    back to unsafe metadata defaults and that v1.4.6 LRET is actually used.
    """
    issues: List[Dict[str, str]] = []
    ...
    # The current scorer does not always echo sentence_count in tier features; do not hard-fail it,
    # but detector_for_scorer must still provide sentence_count.
    ...
    # v1.4.7 scorer evidence gates: when ErrorMap has local errors, scorer cannot
    # report zero chargeable/local pressure.
    ...
    # v1.4.7 Priority gates: raw PE should no longer collapse to UNKNOWN_SKILL
    # after the priority input builder has canonicalized families and task metadata.
    ...
    if _priority_raw_unknown(priority_raw):
        issues.append({"severity": "high", "artifact": "priority", "issue": "raw_priority_unknown_skill_after_v147_priority_input"})

    # v1.4.7 score release policy gate. Gold can still show a student score, but
    # a production-ready learning profile should not silently update if score
    # confidence blocks progress tracking or LIE.
    ...
    # v1.4.6 LRET routing gate.
    lret_version = _lret_engine_version(lret)
    if lret and "1.4.6" not in lret_version:
    ...
    if isinstance(lret, dict) and "clarify_units" not in lret:
        issues.append({"severity": "medium", "artifact": "lret_session", "issue": "lret_v146_clarify_units_field_missing"})

    # v1.4.7 Coach/Directive alignment gate: mismatches are allowed only if the
    # alignment guard has made them explicit.
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Gold full pipeline orchestrator v1.4.6 — standalone orchestration only with downstream quality gates.")
