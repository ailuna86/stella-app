#!/usr/bin/env python3
"""
VA / ST.ELLA Premium (Scored-Only) Pipeline Orchestrator v1.0.0
================================================================

NEW FILE (2026-07-23), forked from gold_full_pipeline_orchestrator_v1_4_9.py
per project convention (never edit a versioned engine file in place). Every
function below is BYTE-IDENTICAL to v1.4.9 except STAGE_ORDER,
REQUIRED_FOR_COMPLETE_GOLD, NON_BLOCKING_STAGES, and the ENGINE_ID/VERSION/
SCHEMA_VERSION constants -- see the diff summary below.

WHY THIS FILE EXISTS: product-owner decision to replace the app's existing
"premium" plan (which used to run a completely separate, older engine set --
pipeline_runner_v14j.py / the full_premium folder's contracts-v2-era Detector/
Feedback engines) with the SAME hardened, verified Gold-quality engines this
session has spent so long fixing (Evaluator-informed Priority Engine,
progress-tracker continuity, etc.) -- just without the coaching/"learning
layer" (Practice, Writing Coach, Vocabulary Coach, Essay Revision, LRET).
Premium still uses an LLM: confirmed directly (grep) that Detector and
Evaluator both make real LLM calls; everything else in this trimmed stage set
(Scorer, Verifier, Adjudicator, Progress Tracker, Feedback Engine, Priority
Engine, LIE) is pure deterministic post-processing, zero LLM, same as in Gold.

WHAT'S REMOVED FROM STAGE_ORDER (vs Gold): lret_session, writing_coach_raw,
writing_coach, practice_session (the Python PracticeEngineV5 bridge -- Gold's
live Practice PAGE never actually consumed this artifact anyway, confirmed
earlier this session; Practice in the premium product is the same
TS-only/exercise-bank feature Gold already uses, no Python engine needed),
mission_response_grading, learning_roadmap (a coaching-routing artifact --
there's nowhere to route to), service_routing (same reason), revision_workspace,
revision_launch_packet, evidence_fusion (its only job is fusing
lret/writing_coach/practice_session outputs, none of which exist here).

WHAT'S KEPT, unchanged: prior_context, intake, detector, detector_for_scorer,
errormap, detector_for_evaluator, metric_profile, evaluator,
evaluator_rubric_bridge, scorer, verifier, adjudicator, score_contract,
progress_tracker, progress_tracker_persist, priority_input, priority,
priority_normalized, directive, feedback_engine, feedback_report,
learner_profile, persisted_profile, skills_progress, progress_snapshot.
skills_progress/progress_snapshot/feedback_report are kept because they are
NOT separate stages/commands at all -- they're side-output files of the
feedback_engine/learner_profile commands (--report-output /
--skills-progress-output / --progress-snapshot-output), so keeping
feedback_engine/learner_profile in the config produces them for free. LIE
(learner_profile) is kept specifically for real cross-essay skill-trend
tracking (product-owner request: "cross-essay tracking + practice exercises
results analysis") -- its --lret/--writing-coach/--practice/--vocabulary-coach
args are simply omitted from the premium engine-commands config (all
documented-optional in gold_lie_profile_builder_standalone_v1_4_7.py), not
pointed at paths that were never produced.

VERIFIED SAFE TO TRIM (not assumed): validate_quality_gates() and
validate_boundaries() below read every excluded-stage artifact through
_safe_obj()/maybe(), both of which return None (not {}) when a file doesn't
exist, and every check on those artifacts is gated behind
`if isinstance(x, dict):` -- so removing these stages produces zero spurious
QA issues; the checks just don't run, exactly like an unconfigured optional
stage in Gold already behaves. The evidence_fusion special-case in main()'s
stage loop is now unreachable (evidence_fusion isn't in STAGE_ORDER anymore)
but deliberately left in place rather than deleted, to keep this diff minimal
and easy to audit against the original.

ORIGINAL v1.4.9 HEADER BELOW (unchanged, for full history) --------------------
...
ENGINE_ID = "VA_STELLA_PREMIUM_SCORED_ORCHESTRATOR"
ENGINE_VERSION = "1.0.0-scored-only-no-learning-layer"
SCHEMA_VERSION = "PREMIUM_SCORED_ORCHESTRATOR_MANIFEST_V1_0"
...
# Removed vs Gold v1.4.9 (see module docstring): lret_session,
# writing_coach_raw, writing_coach, practice_session, mission_response_grading,
# learning_roadmap, service_routing, revision_workspace, revision_launch_packet,
# evidence_fusion -- all coaching/learning-layer or coaching-routing stages,
# or (evidence_fusion) an artifact that only fuses those stages' outputs.

# No stage in this trimmed set is optional/non-blocking the way
# mission_response_grading was in Gold (that stage doesn't exist here at all).
NON_BLOCKING_STAGES: set = set()

# Minimal product-critical artifacts for a complete Premium (scored-only)
# report. Development runs may be partial unless --strict is used. Same list
# as Gold's REQUIRED_FOR_COMPLETE_GOLD minus every learning-layer artifact
# (evidence_fusion, lret_session, writing_coach_raw, writing_coach,
# practice_session, service_routing) -- none of which this orchestrator ever
# produces, so requiring them would make --strict permanently fail.
REQUIRED_FOR_COMPLETE_PREMIUM: List[str] = [
    "prior_context",
    "submission",
    "detector",
    "detector_for_scorer",
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Premium scored-only pipeline orchestrator v1.0.0 — standalone orchestration only, same engines/ordering as Gold minus the coaching/learning-layer stages.")
    ...
    # Vestigial for premium: mission_response_grading isn't in STAGE_ORDER
    # (there's no Writing Coach to generate a mission to grade), so these
    # three args are inert here -- kept only because removing them would mean
    # diverging main()'s body from Gold's more than necessary, and passing
    # them is simply a no-op (see the "mission_response_grading" in commands
    # guard below, which will just never be true for a premium engine-config).
    ...
    # v1.0.0: premium's LIE has a narrower, explicitly-scoped role than Gold's
    # (no Writing Coach/Vocab Coach/Practice-family-accuracy narrative to
    # fold in -- see module docstring) but DOES still cover cross-essay
    # tracking and Practice results, per product decision. Rather than adding
    # a new STAGE_ORDER stage for this (gold_engagement_history_aggregator_v1_0.py
    # reads real session history off disk, it isn't itself a per-submission
    # pipeline stage in Gold either -- goldPipeline.ts calls it directly, ad
    # hoc, the same way it calls the Vocab Coach selection engine), the
    # frontend is expected to call the aggregator itself before invoking this
    # orchestrator and pass the resulting JSON path here. Mirrors the existing
    # --mission-to-grade pattern: an optional flag that becomes an available
    # {engagement_history} template variable, empty string when not supplied
    # (gold_lie_profile_builder_standalone_v1_4_7.py's read_json() already
    # treats an empty/falsy path as "not supplied" and returns None, so this
    # is safe to pass unconditionally from the engine-config template).
    ap.add_argument("--engagement-history", help="Output of gold_engagement_history_aggregator_v1_0.py -- real cross-session Practice/Essay-Revision history for this student, computed by the caller before invoking this orchestrator. Optional; available to commands as {engagement_history}.")
    ...
    template_map.update({
        ...
        "engagement_history": str(Path(args.engagement_history).resolve()) if args.engagement_history else "",
    })
    ...
    missing_required = [k for k in REQUIRED_FOR_COMPLETE_PREMIUM if not artifact_status.get(k, {}).get("exists")]
    invalid_required = [k for k in REQUIRED_FOR_COMPLETE_PREMIUM if artifact_status.get(k, {}).get("exists") and not artifact_status.get(k, {}).get("valid_json")]
