...
def find_covering_task_for_keep(unit: Dict[str, Any], task_units: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    # v1.12.1 fix (session-audit Finding 3): exact text matches no longer require
    # same_sentence() to agree. Confirmed against real data that a KEEP raw
    # candidate and a FIX task unit for the *same underlying phrase region* can
    # carry different source_sentence_index values from different extraction
    # passes -- exact normalized-text equality is itself stronger evidence of
    # "same span" than sentence-index bookkeeping, which is exactly what let
    # "sound mental" survive into keep_units while also sitting in fix_units.
    exact = [t for t in task_units if norm_text(t.get("unit_text")) == norm_text(unit.get("unit_text"))]
    if exact:
...
def covers_unit(coverer: Dict[str, Any], covered: Dict[str, Any], *, loose_fix_overlap: bool = True) -> bool:
    ...
    # v1.12.1 fix (session-audit Finding 3, "sound mental" landing in both
    # fix_units and keep_units simultaneously). A direct token-subsequence or
    # substring relationship between two units' surface text is itself strong,
    # self-contained evidence they refer to the same underlying span -- it does
    # not need same_sentence() to agree first. Confirmed via real session data
    # that a KEEP candidate and a FIX task unit for the same phrase region can
    # be extracted with different source_sentence_index values (different
    # extraction passes / windows), which made same_sentence() the actual
    # point of failure for this class of bug -- the token/substring checks
    # below were already correct, they just never got a chance to run.
    strong_text_overlap = (
        token_sequence_contained(b_seq, a_seq)
        or (norm_text(b_text) and norm_text(b_text) in norm_text(a_text))
    )
    if strong_text_overlap:
        return True

    # Everything past this point is a looser signal (shared tokens / partial
    # overlap ratio) that genuinely does need same_sentence() to avoid false
    # positives across two unrelated sentences that happen to share a word.
    ...
    if len(fix_units) <= _low_corroboration_threshold:
        for unit in fix_units:
            if (unit.get("error_family") == "LEXICAL_PRECISION"
                ...
                # v1.13.0 Gold pipeline fix (Fix 2, part B): mark this unit so the
                # v1.5/v1.6 downstream ENHANCE release gates (_v146_is_valid_final_enhance,
                # _v16_filter_suggestions_for_unit) know to exempt it from their
                # ">=LLM_MIN_VALID_SUGGESTIONS corroborating suggestions" requirement.
                # Root cause this documents: every unit demoted here carries exactly ONE
                # suggestion (its original FIX suggestion) by construction -- it can never
                # satisfy a ">=2 suggestions" gate designed for multi-candidate phrase-enhance
                # units, so without this marker the demotion silently produces a unit that
                # vanishes from BOTH fix_units and enhance_units in the final output (verified
                # directly: confirmed by instrumenting the real analyze() pipeline against the
                # stress_medium session's "the potential downsides" -> "disadvantages" case).
                unit["demoted_from_fix_low_corroboration"] = True
                build_enhance_phase1(unit)
    return fix_units, claimed_spans, suppressed, context_failures
...
ENGINE_VERSION = "lret-engine-v1.12.1-keep-fix-coverage-dedup-fix"  # v1.12.1: session-audit Finding 3/4 fix, see find_covering_task_for_keep/covers_unit above
SCHEMA_VERSION_OUT = "LRET_OUTPUT_V1.5"


# ============================================================================
# v1.13.0 -- Gold pipeline fix pack, built on v1.12.1 (chosen over v1.12.0 as
# the base because v1.12.1's find_covering_task_for_keep/covers_unit fix for
# the "sound mental" KEEP/FIX-duplicate bug is a strict, verified superset of
# v1.12.0 with no regressions -- see the v1.12.1 diff note above). Four fixes,
# each traced to a real session/audit finding:
#
#   Fix 1 (detector-KEEP reconciliation): no code change needed here --
#   _v1610_suppress_keep_units_with_detector_errors() and --detector-output /
#   load_detector_output() already work correctly when wired together
#   (verified directly: re-running v1.12.0/v1.12.1 unchanged against the real
#   "ageing population" session's 01b_errormap_v3.json errormap already
#   suppresses "need modey" out of keep_units). What v1.13.0 changes is
#   operational: this file is registered in gold_engine_commands_full_v1_4_18
#   .json / goldPipeline.ts with --detector-output wired to the errormap
#   artifact for every session (see engine file header run example below and
#   the stella-frontend changelog comment for this version).
#
#   Fix 2 (derive_fix_units's v1.4.13 low-corroboration demotion never
#   surviving to the final output): see _v146_is_valid_final_enhance and
#   _v16_filter_suggestions_for_unit overrides below.
#
#   Fix 3 (same-headword-registry-collocate CLARIFY false positives): see
#   _v1130_llm_judge_vague_candidates and the new _v1100_build_collocation_
#   menus override below.
#
#   Fix 4 (ungrammatical/unnatural ENHANCE substitutions slipping past
#   RuleBasedContextFitValidator's structural gates): see
#   _v1130_apply_enhance_naturalness_check below.
#
# v1.13.1 -- one behavior change on top of v1.13.0: _v1130_apply_enhance_
# naturalness_check was fail-open (no API key configured, or one candidate's
# LLM call failing/timing out, both left that ENHANCE suggestion shown
# unverified -- i.e. a misconfigured deployment would ship exactly the
# ungrammatical substitutions Fix 4 exists to catch). Now fails closed, same
# posture as Fix 3: no verified "grammatical_and_natural" judgment means no
# suggestion is shown, full stop. No other logic changed from v1.13.0.
# ============================================================================


# ----------------------------------------------------------------------------
# Fix 2 (parts C & D): the v1.4.13 FIX->ENHANCE low-corroboration demotion
# (see derive_fix_units above, and the `demoted_from_fix_low_corroboration`
# marker it now sets) was provably dead in practice. Root cause, confirmed by
# instrumenting the real analyze() pipeline end-to-end against the
# stress_medium session's "the potential downsides" -> "disadvantages" case:
# the demotion itself fires correctly (unit.class_label becomes "ENHANCE"),
# but TWO independent downstream ENHANCE release gates each separately
# require >= LLM_MIN_VALID_SUGGESTIONS (default 2) corroborating suggestions
# per unit before letting it through to the final output:
#   1. _v146_is_valid_final_enhance (the v1.5 "final release gate"), and
#   2. _v16_filter_suggestions_for_unit (the v1.6.2 "subject/scope gate").
# Every unit this demotion path produces has EXACTLY ONE suggestion by
# construction (it is still the same single FIX suggestion, just relabeled)
# -- it can never clear a ">=2" bar designed for the multi-candidate
# phrase-enhance-candidate pipeline. Without an exemption, the demoted unit
# silently vanishes from BOTH fix_units and enhance_units in the final
# output -- worse than the original bug, not better. Verified directly with
# and without this fix (see verification notes in this version's changelog).
# ----------------------------------------------------------------------------

_prev_v1130_is_valid_final_enhance = _v146_is_valid_final_enhance


def _v146_is_valid_final_enhance(unit: Dict[str, Any]) -> Tuple[bool, str]:
    if unit.get('demoted_from_fix_low_corroboration'):
        text = str(unit.get('unit_text') or '')
        context = str(unit.get('context') or '')
        malformed, reason = _v146_span_has_grammar_malformation(text)
        if malformed:
        ...
        if context_has_local_grammar_corruption(context) and is_unrecoverable_phrase_fragment(text, context):
            return False, "malformed_clause_repair_not_enhance"
        return True, "ok_demoted_from_fix_exempt_from_min_suggestion_gate"
    return _prev_v1130_is_valid_final_enhance(unit)


_prev_v1130_filter_suggestions_for_unit = _v16_filter_suggestions_for_unit
    ...
    if unit.get('demoted_from_fix_low_corroboration'):
        ...
        for item in unit.get('suggestions') or []:
            ...
            if not ok:
                failures.append({'unit_text': unit_text, 'candidate': text, 'tier': 'v162_subject_scope_gate', 'reason': reason, 'final_sentence': final_sentence})
                continue
            item2 = copy.deepcopy(item) if isinstance(item, dict) else {'text': text}
            item2.setdefault('validation', {}).setdefault('gates', [])
            gates = list(item2['validation'].get('gates') or [])
            if 'v162_subject_scope_retention_checked' not in gates:
                gates.append('v162_subject_scope_retention_checked')
        ...
        unit2['suggestions'] = filtered
        return unit2, filtered
    return _prev_v1130_filter_suggestions_for_unit(unit, validator, failures)


# ----------------------------------------------------------------------------
# Shared LLM helper for the two new v1.13.0 scoped checks (Fix 3, Fix 4).
# Same conventions as _openai_repair_candidates / OpenAILRETSuggestionProvider
# elsewhere in this file: OpenAI Responses API, strict json_schema output,
# bounded retries with linear backoff, failures degrade to "no verdict" for
# the affected candidates (recorded as warnings) rather than raising -- unless
# the caller is run with --llm-required, which is checked in main().
# ----------------------------------------------------------------------------

def _v1130_call_openai_batch(
    candidates: List[Dict[str, Any]], *, system: str, schema: Dict[str, Any], schema_name: str,
    result_key: str, model: str, timeout: int, max_retries: int, retry_sleep: float, batch_size: int = 8,
    ...
    for i in range(0, len(candidates), batch_size):
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                text = OpenAILRETSuggestionProvider._extract_text(data)
                parsed = json.loads(text)
                for row in parsed.get(result_key) or []:
                    if not isinstance(row, dict):
                        continue
        ...
        if last_error is not None:
            _LLM_STATS.setdefault('warnings', []).append(
                f"v1130_llm_batch_call_failed[{schema_name}]: chunk_start={i}; {type(last_error).__name__}: {last_error}"
            )
    return out


_V1130_CLARIFY_LLM_MODEL = "gpt-5-nano"
_V1130_CLARIFY_LLM_TIMEOUT = 120
_V1130_CLARIFY_LLM_MAX_RETRIES = 3
_V1130_CLARIFY_LLM_RETRY_SLEEP = 2.0


# ----------------------------------------------------------------------------
# Fix 3: replace the same-headword-registry-collocate CLARIFY mechanism's
# FINAL judgment with a scoped LLM vagueness check.
#
# Background (see the v1.6.9/v1.6.10 comments above _v165_promote_free_
# modifier_swap_candidates / _v165_promote_free_modifier_swaps for the full
# history): this engine has, at different points in its history, surfaced
# "a same-headword collocate exists in the registry for one word in this
# phrase" as a CLARIFY prompt (safety_level
# 'v169_unverified_registry_modifier_swap_no_asserted_answer' in earlier
# builds; reincarnated in v1.10.0 as the basic-vocab-gated "collocation
# precision menu", safety_level
# 'v1100_registry_attested_collocate_menu_no_single_answer', via
# _v1100_find_menu_items / _v1100_build_collocation_menus). Both are the same
# underlying signal (_v165_generate_free_modifier_swap_candidates: a sibling
# collocate exists in the registry for the SAME headword) and both share the
# exact false-positive failure mode the v1.6.9 comment already diagnosed:
# "attested for the same headword" is a coincidence of collocation data, not
# verified evidence that the phrase now in front of the student is vague or
# that some other word would fit better. Confirmed directly against a real
# web session (the "ageing population" essay): "take care", "family
# traditions", "slow down", "health care", "give good advice" were all
# flagged this way despite being completely clear, correct, idiomatic
# English -- and confirmed against the "strong" stress-test essay that this
# gets WORSE, not better, as vocabulary gets more sophisticated (more
# registry headword collisions, not more actual vagueness).
#
# This fix does NOT touch candidate detection (_v1100_find_menu_items is
# reused unchanged -- it still decides WHICH phrases are candidates). It
# only replaces the final accept/reject decision: instead of unconditionally
# emitting a CLARIFY for every candidate, each candidate's phrase + sentence
# is sent to a scoped LLM call asking whether it is genuinely vague/
# ambiguous in context, or just a normal idiom that happens to share a
# registry headword with something else. Only candidates the LLM confirms
# genuinely vague are emitted. No quota/yield-floor backfill: if every
# candidate in an essay comes back "not vague" (or the LLM is unavailable --
# fail-closed), this mechanism correctly contributes zero CLARIFY units for
# that essay. The OTHER, already-working CLARIFY mechanism (vague-placeholder
# detection via _is_vague_placeholder_phrase / semantic_stability_for_
# enhance) is completely untouched by this fix.
# ----------------------------------------------------------------------------

def _v1130_llm_judge_vague_candidates(candidates: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    if not candidates:
        return {}
    if not _os.environ.get('OPENAI_API_KEY'):
        _LLM_STATS.setdefault('warnings', []).append(
            f"v1130_clarify_vagueness_check_skipped_no_api_key: {len(candidates)} "
            "same-headword-collocate candidate(s) suppressed (fail-closed -- no CLARIFY "
            "emitted from this mechanism without a verified LLM judgment)."
        )
        return {}
    schema = {
        "type": "object",
            ...
            "judgments": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "genuinely_vague": {"type": "boolean"},
                        "reason": {"type": "string"},
                    },
                    "required": ["candidate_id", "genuinely_vague", "reason"],
                },
            }
        },
        "required": ["judgments"],
    ...
    system = (
        "You judge whether short English phrases, as used in a specific sentence from "
        "an IELTS essay, are genuinely vague or ambiguous (a reader would actually be "
        "unsure what the writer means, and the student should be asked to clarify), OR "
        "are normal, clear, idiomatic English that merely happens to share a dictionary "
        "headword with some unrelated collocation. Only mark genuinely_vague=true if a "
        "reader would really be unsure of the intended meaning. Common, correct idioms "
        "and collocations (e.g. 'take care', 'slow down', 'family traditions', 'health "
        "care', 'good advice') are NOT vague just because a thesaurus/collocation "
        "registry lists other words that pair with the same headword. Output JSON only."
    )
    payload_candidates = [{"candidate_id": c["candidate_id"], "phrase": c["unit_text"], "sentence": c["context"]} for c in candidates]
    return _v1130_call_openai_batch(
        payload_candidates, system=system, schema=schema, schema_name="lret_clarify_vagueness_judgments",
        result_key="judgments", model=_V1130_CLARIFY_LLM_MODEL, timeout=_V1130_CLARIFY_LLM_TIMEOUT,
        max_retries=_V1130_CLARIFY_LLM_MAX_RETRIES, retry_sleep=_V1130_CLARIFY_LLM_RETRY_SLEEP,
    )
...
def _v1100_build_collocation_menus(result: Dict[str, Any]) -> None:
    ...
    flags_used = 0
    candidates: List[Dict[str, Any]] = []
    keep_candidates: List[Tuple[Dict[str, Any], List[Dict[str, Any]]]] = []
    for ku in keep_units:
        text = str(ku.get('unit_text') or '')
        if (flags_used >= _V1100_MAX_FLAGS_PER_ESSAY
                or len(surface_tokens(text)) < 2
        ...
        already_flagged_norms.add(norm_text(text))
        flags_used += 1
        cid = f"v1130_keep_{len(keep_candidates):04d}"
        keep_candidates.append((ku, menu_items))
        candidates.append({"candidate_id": cid, "unit_text": text, "context": ku.get('context') or ''})

    clarify_candidates: List[Tuple[Dict[str, Any], List[Dict[str, Any]]]] = []
    for cu in existing_clarify:
        if flags_used >= _V1100_MAX_FLAGS_PER_ESSAY:
            break
        if cu.get('unit_type') not in _V1100_ENRICHABLE_CLARIFY_TYPES or cu.get('suggestions'):
            continue
        text = str(cu.get('unit_text') or '')
        if len(surface_tokens(text)) < 2 or norm_text(text) in already_flagged_norms:
            continue
        ...
        already_flagged_norms.add(norm_text(text))
        flags_used += 1
        cid = f"v1130_clarify_{len(clarify_candidates):04d}"
        clarify_candidates.append((cu, menu_items))
        candidates.append({"candidate_id": cid, "unit_text": text, "context": cu.get('context') or ''})

    judgments: Dict[str, Dict[str, Any]] = _v1130_llm_judge_vague_candidates(candidates) if candidates else {}

    keep_promote_ids: Set[int] = set()
    clarify_added: List[Dict[str, Any]] = []
    for idx, (ku, menu_items) in enumerate(keep_candidates):
        verdict = judgments.get(f"v1130_keep_{idx:04d}")
        if not verdict or not verdict.get('genuinely_vague'):
            continue
        keep_promote_ids.add(id(ku))
        text = str(ku.get('unit_text') or '')
        clarify_added.append({
            'unit_id': ku.get('unit_id') or ("v1100_colloc_menu_%04d" % (len(clarify_added) + 1)),
            'class_label': 'CLARIFY',
            ...
            'context': ku.get('context') or '',
            'source_sentence_index': ku.get('source_sentence_index'),
            'safety_level': 'v1130_llm_verified_genuinely_vague_collocate_slot',
            'reveal_policy': {
                'mode': 'produce_before_reveal',
                'attempt_required_before_suggestions_shown': True,
                'suggestions_role': 'menu_of_other_attested_collocates_not_asserted_correct',
            ...
            'clarify_reason': (
                'This phrase uses a common, basic word choice with registry-attested '
                'alternatives available for the same slot, AND an LLM check confirmed it '
                'reads as genuinely vague/imprecise in this sentence (not just a registry '
                f"coincidence). LLM reason: {verdict.get('reason', '')}"
            ),
            'phase1_prompt': (
                'This is correct, but uses a common word choice: [' + text + ']. Try '
                'rewriting it yourself with a more precise or natural-sounding alternative first.'
    ...
    result['keep_units'] = [ku for ku in keep_units if id(ku) not in keep_promote_ids]

    enriched_count = 0
    for idx, (cu, menu_items) in enumerate(clarify_candidates):
        verdict = judgments.get(f"v1130_clarify_{idx:04d}")
        if not verdict or not verdict.get('genuinely_vague'):
            continue
        text = str(cu.get('unit_text') or '')
        enriched_count += 1
        cu['unit_type'] = 'collocation_precision_menu'
        cu['safety_level'] = 'v1130_llm_verified_genuinely_vague_collocate_slot'
        cu['reveal_policy'] = {
            'mode': 'produce_before_reveal',
            'attempt_required_before_suggestions_shown': True,
            'suggestions_role': 'menu_of_other_attested_collocates_not_asserted_correct',
        ...
        cu['clarify_reason'] = (
            'This phrase uses a common, basic word choice with registry-attested '
            'alternatives available for the same slot, AND an LLM check confirmed it '
            'reads as genuinely vague/imprecise in this sentence. LLM reason: '
    ...
    result.setdefault('lexical_profile', {})['v1100_collocation_precision_menu'] = {
        ...
        'menus_flagged_from_existing_clarify': enriched_count,
        'menus_flagged_total': len(clarify_added) + enriched_count,
        'v1130_candidates_considered': len(candidates),
        'v1130_llm_confirmed_genuinely_vague': len(clarify_added) + enriched_count,
        'v1130_llm_available': bool(_os.environ.get('OPENAI_API_KEY')),
    }


_V1130_ENHANCE_LLM_MODEL = "gpt-5-nano"
_V1130_ENHANCE_LLM_TIMEOUT = 120
_V1130_ENHANCE_LLM_MAX_RETRIES = 3
_V1130_ENHANCE_LLM_RETRY_SLEEP = 2.0


# ----------------------------------------------------------------------------
# Fix 4: final LLM grammar/naturalness validation for ENHANCE substitutions.
#
# RuleBasedContextFitValidator's gates (span_fit, grammar_role_preserved,
# meaning_link_detected, claim_strength_preserved, register_preserved_or_
# improved, context_specificity_ok, no_topic_drift, no_unsafe_word) are all
# structural/token-level checks. Confirmed directly they do not catch: (a)
# "people often help" -> "people often facilitate" / "people often enable"
# (ungrammatical once substituted -- "facilitate"/"enable" don't take a bare
# infinitive complement the way "help" does; they need a different structure
# such as "enable people to do X"), or (b) "not only good grades" -> "not
# only beneficial grades" / "not only favourable grades" (grammatical, but
# not a natural English collocation). This adds one more check, after every
# other gate, asking an LLM directly whether the SUBSTITUTED sentence reads
# as grammatically correct AND natural -- not whether the replacement word is
# plausible in isolation. Any suggestion that fails is dropped; a unit left
# with zero surviving suggestions is dropped from enhance_units entirely
# (and both are recorded under qa.source_audit for auditability). Fails open
# (skips the check, leaves suggestions as-is) if no OPENAI_API_KEY is
# available, matching this file's existing posture for optional LLM steps
# elsewhere (e.g. _openai_repair_candidates) -- see this version's
# changelog/verification notes for the residual risk this implies.
# ----------------------------------------------------------------------------

def _v1130_apply_enhance_naturalness_check(result: Dict[str, Any]) -> None:
    enhance_units = result.get('enhance_units') or []
    if not enhance_units:
        return
    candidates: List[Dict[str, Any]] = []
    lookup: Dict[str, Tuple[Dict[str, Any], int]] = {}
    for u in enhance_units:
        unit_id = str(u.get('unit_id') or id(u))
        sentence = str(u.get('context') or '')
        original = str(u.get('unit_text') or '')
        for s_idx, s in enumerate(u.get('suggestions') or []):
            text = str((s or {}).get('text') if isinstance(s, dict) else s or '').strip()
            if not text:
                continue
            cid = f"{unit_id}::{s_idx}"
            lookup[cid] = (u, s_idx)
    ...
    if not candidates:
        return
    # v1.13.1: was fail-open (returned here, leaving every suggestion shown
    # unverified) -- an essay's ENHANCE suggestions could then ship with the
    # exact ungrammatical substitutions this check exists to catch (e.g.
    # "help" -> "facilitate"/"enable"), silently, in any deployment missing
    # OPENAI_API_KEY. Now fails closed: no API key means no candidate gets a
    # real judgment, so every one of them is dropped below (verdict is None
    # is handled the same as a negative verdict), matching Fix 3's posture.
    has_api_key = bool(_os.environ.get('OPENAI_API_KEY'))
    if not has_api_key:
        result.setdefault('qa', {}).setdefault('warnings', []).append(
            f"v1131_enhance_naturalness_check_no_api_key: {len(candidates)} ENHANCE "
            "suggestion(s) could not be verified for grammar/naturalness and were "
            "suppressed (fail-closed -- no ENHANCE suggestion is shown without a "
            "verified LLM judgment)."
        )
        ...
        "properties": {
            "judgments": {
                ...
                "items": {
                    ...
                    "properties": {
                        "candidate_id": {"type": "string"},
                        "grammatical_and_natural": {"type": "boolean"},
                        "reason": {"type": "string"},
                    },
                    "required": ["candidate_id", "grammatical_and_natural", "reason"],
                },
            }
        },
        "required": ["judgments"],
    ...
    system = (
        "You check whether substituting a proposed replacement phrase for the original "
        "phrase, in the exact position it occupies in the given sentence, produces a "
        "result that is BOTH grammatically correct AND reads as natural, idiomatic "
        "English -- not just plausible in isolation. Common failure to catch: a verb "
        "that does not take the same complement pattern as the original (e.g. 'help do "
        "X' is fine but 'facilitate do X' / 'enable do X' is not -- those verbs need a "
        "different structure, such as 'enable people to do X'), and collocations that "
        "are grammatical but simply not natural English (e.g. 'beneficial grades', "
        "'favourable grades' in place of 'good grades'). Judge the SUBSTITUTED "
        "sentence as a whole, not the replacement word in isolation. Output JSON only."
    )
    # v1.13.1: only call out when we actually have a key -- with no key, every
    # candidate simply gets no judgment (judgments stays {}), which the loop
    # below now treats as a failure to verify, not a pass.
    judgments = _v1130_call_openai_batch(
        candidates, system=system, schema=schema, schema_name="lret_enhance_naturalness_judgments",
        result_key="judgments", model=_V1130_ENHANCE_LLM_MODEL, timeout=_V1130_ENHANCE_LLM_TIMEOUT,
        max_retries=_V1130_ENHANCE_LLM_MAX_RETRIES, retry_sleep=_V1130_ENHANCE_LLM_RETRY_SLEEP,
    ) if has_api_key else {}

    dropped_suggestions: List[Dict[str, Any]] = []
    to_drop: Dict[int, Set[int]] = defaultdict(set)
    for cid, (u, s_idx) in lookup.items():
        verdict = judgments.get(cid)
        suggs = u.get('suggestions') or []
        # v1.13.1: was `continue` here (fail open -- an LLM call that failed
        # or was never made, e.g. a timeout on one candidate in an otherwise-
        # successful batch, left that suggestion shown unverified). Now
        # treated the same as an explicit "not grammatical_and_natural"
        # verdict: no verified judgment means no suggestion shown.
        if verdict is None:
            to_drop[id(u)].add(s_idx)
            dropped_suggestions.append({
                'unit_id': u.get('unit_id'),
                'unit_text': u.get('unit_text'),
                'suggestion': suggs[s_idx].get('text') if s_idx < len(suggs) and isinstance(suggs[s_idx], dict) else None,
                'reason': 'no_verified_llm_judgment_available' if has_api_key else 'no_api_key_configured',
            })
            continue
        if not verdict.get('grammatical_and_natural'):
            to_drop[id(u)].add(s_idx)
            dropped_suggestions.append({
                'unit_id': u.get('unit_id'),
                'unit_text': u.get('unit_text'),
    ...
    survivors: List[Dict[str, Any]] = []
    fully_dropped: List[Dict[str, Any]] = []
    for u in enhance_units:
        drop_idx = to_drop.get(id(u))
        if drop_idx:
            u['suggestions'] = [s for i, s in enumerate(u.get('suggestions') or []) if i not in drop_idx]
        if u.get('suggestions'):
            survivors.append(u)
        else:
            fully_dropped.append(u)

    result['enhance_units'] = survivors
    dropped_ids = {u.get('unit_id') for u in fully_dropped}
    result['replacement_options'] = [
        ro for ro in (result.get('replacement_options') or [])
        if not (ro.get('class_label') == 'ENHANCE' and ro.get('unit_id') in dropped_ids)
    ]
    qa_audit = result.setdefault('qa', {}).setdefault('source_audit', {})
    qa_audit.setdefault('llm_naturalness_check_failures', []).extend(dropped_suggestions)
    qa_audit['llm_naturalness_check_units_dropped'] = qa_audit.get('llm_naturalness_check_units_dropped', 0) + len(fully_dropped)
    result.setdefault('lexical_profile', {})['enhance_count'] = len(survivors)


_prev_main_v1130 = main


def main(argv: Optional[List[str]] = None) -> int:
    global ACTIVE_LLM_PROVIDER, LLM_MAX_CANDIDATES, LLM_MIN_VALID_SUGGESTIONS
    global _V1130_CLARIFY_LLM_MODEL, _V1130_CLARIFY_LLM_TIMEOUT, _V1130_CLARIFY_LLM_MAX_RETRIES, _V1130_CLARIFY_LLM_RETRY_SLEEP
    global _V1130_ENHANCE_LLM_MODEL, _V1130_ENHANCE_LLM_TIMEOUT, _V1130_ENHANCE_LLM_MAX_RETRIES, _V1130_ENHANCE_LLM_RETRY_SLEEP
    parser = argparse.ArgumentParser(
        description="LRET Engine v1.13.1 -- LLM-verified CLARIFY vagueness + ENHANCE naturalness checks "
                    "(both fail-closed), plus the v1.4.13 FIX->ENHANCE demotion release-gate fix")
    parser.add_argument("--input", "-i", required=True, help="Path to LRET input JSON or full Evaluator/WKE output JSON")
    parser.add_argument("--output", "-o", required=True, help="Path to write LRET output JSON")
    parser.add_argument("--mode", choices=["fix_only", "enhance_only", "fix_and_enhance"], default=None)
    parser.add_argument("--student-id", default=None)
    ...
    parser.add_argument("--canonical-resources", default=None, help="Path to canonical resources directory or final_app_registries zip")
    parser.add_argument("--detector-output", default=None,
                         help="Path to the Detector's output/errormap JSON (per-essay 01b_errormap_v3.json is the "
                              "recommended format -- it carries the top-level 'errors' array that "
                              "_v1610_suppress_keep_units_with_detector_errors() needs; batch 01_detector_output.json "
                              "is also accepted for validated_fix_candidates/broken_sentences_raw). Gold pipeline "
                              "invocations should always pass this -- see Fix 1 in this version's changelog.")
    parser.add_argument("--disable-repeated-word-check", action="store_true",
                         help="Disable the v1.9.0 repeated-generic-word CLARIFY prompts (on by default).")
    parser.add_argument("--disable-collocation-menu-check", action="store_true",
                         help="Disable the v1.10.0/v1.13.0 collocation-slot precision menu CLARIFY prompts (on by default).")
    parser.add_argument("--disable-grammar-flag-check", action="store_true",
                         help="Disable the v1.11.0 malformed-sentence visibility flags (on by default).")
    parser.add_argument("--disable-truncated-vp-fix", action="store_true",
                         help="Disable the v1.11.0 truncated-VP KEEP->CLARIFY fix (on by default).")
    parser.add_argument("--disable-enhance-naturalness-check", action="store_true",
                         help="Disable the v1.13.0 LLM grammar/naturalness check on ENHANCE suggestions (on by default).")
    parser.add_argument("--use-llm", action="store_true",
                         help="Allow an LLM pass IF the free registry-only pass under-delivers (see --llm-yield-floor). "
                              "Does not force an LLM call by itself. Also gates the v1.13.0 CLARIFY-vagueness and "
                              "ENHANCE-naturalness checks (in addition to requiring OPENAI_API_KEY).")
    parser.add_argument("--force-llm", action="store_true",
                         help="Run the LLM pass unconditionally (skips the yield-floor check). Implies --use-llm.")
    parser.add_argument("--llm-yield-floor", type=int, default=6,
                         help="Minimum (enhance_count + asserted contextual_synonym_count) from the free pass "
    ...
    parser.add_argument("--llm-required", action="store_true", help="Fail if the LLM pass triggers but OPENAI_API_KEY is missing or the request fails")
    parser.add_argument("--llm-model", default="gpt-5-nano",
                         help="OpenAI model for suggestion generation, and default model for the v1.13.0 "
                              "CLARIFY-vagueness/ENHANCE-naturalness checks; default gpt-5-nano ($0.05/$0.40 per 1M "
                              "input/output tokens vs gpt-5-mini's $0.25/$2.00) -- every suggestion is still "
                              "deterministically re-validated regardless of model, so nano-tier is the "
                              "recommended default; override if quality in practice requires more.")
    parser.add_argument("--llm-timeout", type=int, default=120, help="OpenAI read timeout in seconds; increase on slow connections")
    ...
    LLM_MAX_CANDIDATES = max(0, int(args.llm_max_candidates))
    LLM_MIN_VALID_SUGGESTIONS = max(1, int(args.llm_min_valid_suggestions))
    _V1130_CLARIFY_LLM_MODEL = args.llm_model
    _V1130_CLARIFY_LLM_TIMEOUT = args.llm_timeout
    _V1130_CLARIFY_LLM_MAX_RETRIES = args.llm_max_retries
    _V1130_CLARIFY_LLM_RETRY_SLEEP = args.llm_retry_sleep
    _V1130_ENHANCE_LLM_MODEL = args.llm_model
    _V1130_ENHANCE_LLM_TIMEOUT = args.llm_timeout
    _V1130_ENHANCE_LLM_MAX_RETRIES = args.llm_max_retries
    _V1130_ENHANCE_LLM_RETRY_SLEEP = args.llm_retry_sleep
    load_external_lexical_resources(args.resources)
    load_canonical_resources(args.canonical_resources)
    load_detector_output(args.detector_output)
    ACTIVE_LLM_PROVIDER = None
    ...
    if not args.disable_repeated_word_check:
        _v190_flag_repeated_generic_words(result)
    if not args.disable_collocation_menu_check:
        # v1.13.0 (Fix 3): this now runs the LLM-gated vagueness check
        # (_v1130_llm_judge_vague_candidates) instead of unconditionally
        # emitting a CLARIFY for every same-headword-registry-collocate
        # candidate -- only fires the LLM check at all if --use-llm/
        # --force-llm was passed AND OPENAI_API_KEY is set; otherwise it is a
        # no-op for this mechanism (fail-closed: zero CLARIFY units from
        # this specific mechanism, not a fallback to the old heuristic).
        if llm_allowed and _os.environ.get('OPENAI_API_KEY'):
            _v1100_build_collocation_menus(result)
    if not args.disable_truncated_vp_fix:
        _v1110_fix_truncated_vp_keep(result)
    if not args.disable_grammar_flag_check:
        ...
        _v1110_llm_active = bool(_LLM_STATS.get('calls', 0))
        _v1110_malformed_sentence_flags(result, lret_input.get('essay_text') or '', llm_active=_v1110_llm_active)
    if not args.disable_enhance_naturalness_check:
        # v1.13.0 (Fix 4): same gating philosophy as Fix 3 above -- only runs
        # if LLM use is permitted for this invocation and a key is available;
        # otherwise fails open (leaves ENHANCE suggestions unvalidated for
        # grammar/naturalness -- see the fail-open rationale in this
        # function's docstring/changelog note above).
        if llm_allowed and _os.environ.get('OPENAI_API_KEY'):
            _v1130_apply_enhance_naturalness_check(result)
    result['qa'].setdefault('contract_checks', {})['llm_does_not_create_new_spans'] = True
    result['qa'].setdefault('contract_checks', {})['llm_suggestions_deterministically_validated'] = True
    result['qa'].setdefault('contract_checks', {})['no_embedded_phrase_enhance_bank'] = True
    result['qa'].setdefault('contract_checks', {})['canonical_resources_external_only'] = bool(args.canonical_resources)
    ...
    if args.summary:
        ...
        v1110g = p.get('v1110_malformed_sentence_flags', {})
        v1110t = p.get('v1110_truncated_vp_fix', {})
        qa_audit = result.get('qa', {}).get('source_audit', {})
        print("=== LRET v1.13.1 summary (LLM-verified CLARIFY + ENHANCE checks, both fail-closed) ===")
        print("output:", args.output)
        print("fix_units:", p.get("fix_count"))
        print("main_enhance_units:", p.get("enhance_count"))
        print("contextual_synonym_tasks:", p.get("contextual_synonym_task_count"))
        ...
        print("llm_triggered:", v167.get("triggered"))
        print("canonical_loaded:", p.get("canonical_resource_stats", {}).get("canonical_loaded"))
        print("keep_units_suppressed_for_detector_error_overlap:", p.get("keep_units_suppressed_for_detector_error_overlap"))
        print("repeated_words_flagged:", v190.get("words_flagged"))
        print("collocation_menus_flagged_total:", v1100.get("menus_flagged_total"))
        print("v1130_clarify_candidates_considered:", v1100.get("v1130_candidates_considered"))
        print("v1130_clarify_llm_available:", v1100.get("v1130_llm_available"))
        print("malformed_sentences_flagged:", v1110g.get("sentences_flagged"))
        print("truncated_vp_moved_from_keep:", v1110t.get("units_moved_from_keep"))
        print("v1130_enhance_naturalness_units_dropped:", qa_audit.get("llm_naturalness_check_units_dropped", 0))
        print("v1130_enhance_naturalness_suggestions_dropped:", len(qa_audit.get("llm_naturalness_check_failures", []) or []))
        print("qa_status:", result.get("qa", {}).get("status"))
        print("qa_warnings:", len(result.get("qa", {}).get("warnings", [])))
    return 0


ENGINE_VERSION = "lret-engine-v1.13.1-enhance-fail-closed"
SCHEMA_VERSION_OUT = "LRET_OUTPUT_V1.6"
