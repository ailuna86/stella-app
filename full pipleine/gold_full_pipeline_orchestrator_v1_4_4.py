...
ENGINE_VERSION = "1.4.4-scorer-metadata-lret144-quality-gated-orchestration-only-standalone"
...
def validate_quality_gates(paths: Dict[str, Path]) -> List[Dict[str, str]]:
    ...
    back to unsafe metadata defaults and that v1.4.4 LRET is actually used.
    """
    issues: List[Dict[str, str]] = []
    ...
    # v1.4.4 LRET routing gate.
    lret_version = _lret_engine_version(lret)
    if lret and "1.4.4" not in lret_version:
    ...
    if isinstance(lret, dict) and "clarify_units" not in lret:
        issues.append({"severity": "medium", "artifact": "lret_session", "issue": "lret_v144_clarify_units_field_missing"})
    ...
    # by a later service-alignment release, but v1.4.4 focuses on scorer metadata and LRET routing.
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="Gold full pipeline orchestrator v1.4.4 — standalone orchestration only with downstream quality gates.")
