...
def _v1130_apply_enhance_naturalness_check(result: Dict[str, Any]) -> None:
    ...
    if not _os.environ.get('OPENAI_API_KEY'):
        result.setdefault('qa', {}).setdefault('warnings', []).append(
            f"v1130_enhance_naturalness_check_skipped_no_api_key: {len(candidates)} ENHANCE "
            "suggestion(s) not verified for grammar/naturalness; shown as-is (fail-open, same "
            "posture as other optional LLM steps in this engine)."
        )
        return
    ...
    for cid, (u, s_idx) in lookup.items():
        ...
        if verdict is None:
            continue  # LLM call failed for this candidate -- fail open, leave suggestion as-is
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="LRET Engine v1.13.0 -- LLM-verified CLARIFY vagueness + ENHANCE naturalness checks, "
                    "plus the v1.4.13 FIX->ENHANCE demotion release-gate fix")
    parser.add_argument("--input", "-i", required=True, help="Path to LRET input JSON or full Evaluator/WKE output JSON")
    parser.add_argument("--output", "-o", required=True, help="Path to write LRET output JSON")
    parser.add_argument("--mode", choices=["fix_only", "enhance_only", "fix_and_enhance"], default=None)
    parser.add_argument("--student-id", default=None)
    ...
    if args.summary:
        ...
        print("=== LRET v1.13.0 summary (LLM-verified CLARIFY + ENHANCE checks) ===")
        print("output:", args.output)
        print("fix_units:", p.get("fix_count"))
        print("main_enhance_units:", p.get("enhance_count"))
        print("contextual_synonym_tasks:", p.get("contextual_synonym_task_count"))
...
ENGINE_VERSION = "lret-engine-v1.13.0-llm-verified-clarify-and-enhance"
