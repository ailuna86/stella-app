#!/usr/bin/env python3
"""
Gold LIE Profile Builder v1.4.4 — standalone
============================================
...
CHANGE FROM v1.4.3 (additive only -- no existing field, arg, or computation
removed or altered):
  New optional `--vocabulary-coach PATH` input, pointing at a
  `{student_id}_vocab_coach_ledger.json` artifact (written by
  `vocab_coach_ledger_update_v1_1.py`). When present, this profile now
  surfaces vocabulary-acquisition progress alongside the existing
  grammar/structure progress, per
  GOLD_PIPELINE_SPEC_V4_VOCABULARY_COACH.md Section 7a's "Feeds" table
  ("Learner profile contribution... closes part of the Finding-5 'progress
  tracker' gap -- vocabulary gains should show up there, not just live in
  Vocabulary Coach's own ledger"). When absent, every new field degrades to
  null/empty exactly as the other optional artifacts already do -- no
...
SCHEMA_VERSION = "GOLD_LEARNER_PROFILE_STANDALONE_V1_4_4"
ENGINE_ID = "VA_STELLA_GOLD_LIE_PROFILE_BUILDER"
ENGINE_VERSION = "1.4.4-standalone-no-imports"
...
def vocabulary_coach_summary(vocab_coach: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """New in v1.4.4. Summarizes the Vocabulary Coach ledger (Leitner-box
...
def build(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    vocab_coach = read_json(args.vocabulary_coach, required=False)  # new in v1.4.4
    ...
    vocab_summary = vocabulary_coach_summary(vocab_coach)  # new in v1.4.4
    profile = {
        ...
        "service_artifact_status": {
            ...
            "vocabulary_coach": artifact_status(vocab_coach),  # new in v1.4.4
        },
        "vocabulary_coach_summary": vocab_summary,  # new in v1.4.4 -- None if not provided
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument("--vocabulary-coach")  # new in v1.4.4, optional
    ...
    skills = {
        "schema_version": "GOLD_SKILLS_PROGRESS_STANDALONE_V1_4_4",
        "created_at": now_iso(),
        ...
        "vocabulary_coach_summary": profile.get("vocabulary_coach_summary"),  # new in v1.4.4
        "boundary": "Progress view derived from latest profile evidence only.",
    }
    roadmap = {
        "schema_version": "GOLD_LEARNING_ROADMAP_STANDALONE_V1_4_4",
        "created_at": now_iso(),
        "student_id": profile.get("student_id"),
        "roadmap": [
    ...
    progress = {
        "schema_version": "GOLD_PROGRESS_SNAPSHOT_STANDALONE_V1_4_4",
        "created_at": now_iso(),
        "student_id": profile.get("student_id"),
