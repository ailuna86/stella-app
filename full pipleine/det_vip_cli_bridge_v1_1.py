...
Standalone Gold-contract CLI wrapper around det_vip_v18d_3_topic_alignment_risk.py's
(v1.1: same PremiumDetectorV9, plus the new topic_alignment_risk safety-net field)
...
ENGINE_VERSION = "1.1.0-wraps-det_vip_v18d_3-topic-alignment-risk-passthrough"
...
def build_bridge_output(
    ...
    metric_profile.setdefault("source", "det_vip_v18d_3_topic_alignment_risk_via_bridge")
    ...
    return {
        ...
        "wrapped_engine": "det_vip_v18d_3_topic_alignment_risk.py::PremiumDetectorV9",
...
def main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(description="CLI bridge for det_vip_v18d_3_topic_alignment_risk.py's PremiumDetectorV9 (LLM-based detector, gpt-4o-mini default). v1.1: passes through the new topic_alignment_risk safety-net field (GOLD_PIPELINE_SPEC_V3_TASK_RELEVANCE.md Section 3).")
    ...
    ap.add_argument("--engine-module-dir", required=False, help="Directory containing det_vip_v18d_3_topic_alignment_risk.py, if not already importable.")
    args = ap.parse_args(argv)

    if args.engine_module_dir:
        sys.path.insert(0, args.engine_module_dir)
    try:
        from det_vip_v18d_3_topic_alignment_risk import PremiumDetectorV9
    except ImportError as exc:
        raise SystemExit(
            f"Could not import PremiumDetectorV9 from det_vip_v18d_3_topic_alignment_risk.py "
