...
ENGINE_ID       = "VA_PREMIUM_EVALUATOR_WKE_V8_4"
ENGINE_VERSION  = "8.4.0-session-audit-finding1-task-relevance-fix"
...
def _depth_claim(skill_id: str, maps: Dict, base: Dict) -> int:
    ...
    if skill_id in ("arg_claim_relevance",):
        # v8.4 fix (session-audit Finding 1): this used to return DEPTH_2
        # whenever any claim existed, regardless of whether that claim
        # actually addressed the assigned prompt -- confirmed live to give a
        # 100%-off-topic essay full marks here. This structural heuristic has
        # no access to the prompt and cannot judge topical relevance by
        # itself, so its ceiling must be conservative (DEPTH_1, "a claim
        # exists but relevance is unverified"), not DEPTH_2 ("verified
        # relevant"). The real relevance judgment happens in the LLM
        # refinement pass (see LLM_SYSTEM_V72 rule 7 and _build_llm_payload's
        # assigned_prompt field), which can lower this further to DEPTH_0 for
        # genuinely off-topic essays, or confirm DEPTH_1/DEPTH_2 when it can
        # actually compare the essay against the prompt. When use_llm is off
        # (no API key, --no-llm), this DEPTH_1 ceiling is what ships --
        # honestly uncertain, not a false DEPTH_2.
        return DEPTH_1 if claims else DEPTH_0
...
def estimate_skill_depth(skill_id: str, maps: Dict, base: Dict, is_ad: bool) -> int:
    ...
    if "counterargument" in sid or "rebuttal" in sid:
        return _depth_rebuttal(maps, base, is_ad)
    # v8.4 fix (session-audit Finding 1): maintain_task_focus previously
    # fell through into _depth_generic's broad "task"/"genre"/"purpose"/
    # "audience"/"prompt"/"component" keyword bucket, which is also where
    # genuinely structural skills like identify_genre live (essay vs. letter
    # vs. report -- a real structural question with no relevance dimension).
    # Carving this one skill_id out with its own conservative ceiling, rather
    # than lowering the whole shared bucket, avoids incorrectly under-scoring
    # identify_genre and similar skills that were never the bug. Same
    # reasoning as arg_claim_relevance in _depth_claim: counting claims/
    # paragraphs cannot verify whether the essay's focus matches the assigned
    # prompt, so DEPTH_1 is the honest structural ceiling; the LLM refinement
    # pass (assigned_prompt-aware) does the real relevance check and may
    # lower it to DEPTH_0 or confirm higher.
    if sid == "maintain_task_focus":
        cc = base["claim_candidate_count"]; pc = base["paragraph_count"]
        if cc >= 1 or pc >= 1:
            return DEPTH_1
        return DEPTH_0
...
7. TASK-RELEVANCE CHECK (v8.4 fix, session-audit Finding 1): for skills that
   measure whether the essay's argument actually addresses the assigned prompt
   — this includes at minimum arg_claim_relevance and maintain_task_focus, and
   any other skill whose definition mentions "relevant to the essay prompt" or
   "task focus" — you MUST compare the essay's actual subject matter against
   the assigned_prompt field provided below, not just check whether claims
   exist or the essay is internally well-argued. A fluent, well-organized,
   clearly-argued essay that is about a DIFFERENT topic than assigned_prompt is
   OFF-TOPIC and must score DEPTH_0 on these skills regardless of how good the
   writing is otherwise. An essay that addresses the general subject area but
   misses the specific angle/position the prompt asks for is TANGENTIAL and
   scores DEPTH_1, not DEPTH_2. Only essays that substantively engage with what
   assigned_prompt actually asks score DEPTH_2. Do not let essay fluency or
   argument quality on the wrong topic raise this depth — a well-written essay
   about the wrong thing is still wrong for these specific skills, even though
   it may still score normally on skills unrelated to task relevance (grammar,
   cohesion, etc. are unaffected by this rule).
...
def _build_llm_payload(
    ...
    return {
        "instruction": (
            "Assess each skill independently using the essay text, the assigned prompt, and "
            "candidate evidence. Return depth_assessment and status_recommendation for each. "
            ...
            "See system rule 7 for task-relevance skills specifically — compare the essay's "
            "actual content against assigned_prompt, don't just check argument quality."
        ),
        # v8.4 fix (session-audit Finding 1): prompt_text existed on
        # EvaluatorRequest and was already threaded into extract_text_maps()
        # for structural parsing, but was never included in the LLM
        # refinement payload -- meaning even with use_llm on, the model
        # judging arg_claim_relevance/maintain_task_focus had no way to know
        # what the assigned prompt actually was, only the essay text. This is
        # the fix: give it the prompt so it can actually check relevance.
        "assigned_prompt": (prompt_text or "")[:1500],
...
def call_llm_refinement(
    ...
    payload = _build_llm_payload(
        selected, req.essay_text, maps.get("evidence_index",{}), depth_cache,
