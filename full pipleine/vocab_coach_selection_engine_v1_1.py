...
CHANGES FROM v1_0 (bug-fix pass only -- no selection/rotation/bias logic
touched):

1. Required-file-load robustness fix: `load_json()` was applied uniformly to
   --topic-bank/--task-type-bank/--prompt-bank (required) and
   --score-contract (optional) alike, silently returning None on a
   missing/bad path in every case. For the three REQUIRED banks this meant a
   missing/mistyped path produced a confusing downstream
   `TypeError: 'NoneType' object is not subscriptable` inside
   enumerate_units() instead of a clear message. main() now validates each
   required bank immediately after loading and raises
   `SystemExit(f"Required file not found or invalid: {path}")` if it is None,
   before any other logic runs. --score-contract is untouched (still
   optionally None, as designed).

2. Docstring correction: v1_0's SCOPE NOTE stated "Per explicit user
   instruction, this build proceeds as 'backend engines only, fully
   standalone'". That is not what was decided -- the actual recorded answer
   to the build-scope clarifying question was "Full stack together" (full
   backend + frontend + pipeline wiring in one pass). This file's own
   internal logic does not depend on that claim either way (it is a pure,
   standalone CLI script regardless), so nothing behavioural changed --  only
   the inaccurate docstring claim is corrected here, since leaving a false
   "per explicit user instruction" statement in a project file is worse than
   removing it. The "files referenced by the build prompt were not found in
   this environment" observation is kept, since that part was independently
   re-verified against the real project structure -- but is now stated as an
   environment/tooling limitation of whatever process produced this file,
   not as a user decision.

No other code was changed: rotation, LRET-bias, Leitner-adjacent scoring,
candidate filtering/selection, and output schema are byte-identical to v1_0's
logic. v1_0 is left untouched per project convention (new versioned file,
...
ENGINE_VERSION = "vocab-coach-selection-engine-v1.1"
DEFAULT_COOLDOWN_HOURS = 24.0

CEFR_ORDER = ["A1", "A2", "B1", "B2", "C1", "C2"]

# ---------------------------------------------------------------------------
# LRET family mapping -- grounded in the real v1.12.x LRET output schema
# (fix_units[].error_family, clarify_units[].unit_type,
...
def require_json(path, label):
    ...
    function crash with an opaque TypeError. (Fix in v1_1 -- see module
    docstring, change 1.)"""
...
# ---------------------------------------------------------------------------
# Score-contract level gating (resilient multi-key lookup, documented
# fail-safe to mid-band if no field is recognised)
# ---------------------------------------------------------------------------
...
def main():
    ...
    # v1_1 fix: required banks now raise a clear error on a bad/missing path
    # instead of silently becoming None (see require_json + module docstring).
    ...
    score_contract = load_json(args.score_contract, default=None)  # optional, unchanged
