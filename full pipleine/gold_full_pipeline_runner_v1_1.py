...
# -*- coding: utf-8 -*-
"""
VA / ST.ELLA — Gold Full Pipeline Runner v1.1
=============================================
...
SCHEMA_VERSION = "GOLD_FULL_PIPELINE_OUTPUT_V1_1"
...
def load_submission(path: Path, essay_index: int = 0) -> Dict[str, Any]:
    """Load one essay submission.

    Accepted input shapes:
      1) Single-essay object:
         {"essay_id": "...", "student_id": "...", "essay_text": "...", ...}

      2) Batch wrapper:
         {"essays": [{"essay_id": "...", "student_id": "...", "essay_text": "...", ...}]}

    Gold runner v1.1 selects one essay from a batch using --essay-index.
    Full multi-essay batch execution should be handled by a separate batch wrapper
    so each essay receives an independent Premium/Gold session and learner update.
    """
    raw = read_json(path, required=True)
    ...
    if isinstance(raw.get("essays"), list):
        essays = [e for e in raw.get("essays", []) if isinstance(e, dict)]
        if not essays:
            raise ValueError("Submission JSON has an essays list, but it contains no essay objects.")
        if essay_index < 0 or essay_index >= len(essays):
            raise ValueError(f"--essay-index {essay_index} is out of range for essays list of length {len(essays)}.")
        submission = dict(essays[essay_index])
        source_shape = "batch.essays"
        # Inherit useful batch-level fields only when the selected essay lacks them.
        for key in ("student_id", "prompt_text", "task_type", "topic"):
            if not clean_text(submission.get(key)) and clean_text(raw.get(key)):
                submission[key] = raw.get(key)
        submission["source_batch_path"] = str(path)
    ...
    if not clean_text(submission.get("essay_text")):
        raise ValueError(
            "Submission JSON must contain non-empty essay_text. "
            "Use either a single essay object with essay_text at the top level, "
            "or a batch object with essays[0].essay_text."
        )
...
class GoldPipelineRunner:
    def __init__(self, args: argparse.Namespace):
        ...
        self.submission = load_submission(self.input_path, essay_index=int(getattr(args, "essay_index", 0) or 0))
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="VA/ST.ELLA Gold Full Pipeline Runner v1.1")
    p.add_argument("--input", required=True, help="Submission JSON. Accepts a single essay object or a batch object with essays[].")
    p.add_argument("--essay-index", type=int, default=0, help="When --input contains {essays:[...]}, select this zero-based essay index. Default: 0.")
