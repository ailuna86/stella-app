...
def _v165_promote_free_modifier_swaps(result: Dict[str, Any]) -> None:
    ...
    for ku in keep_units:
        ...
        clarify_added.append({
            'unit_id': ku.get('unit_id') or f"v169_clarify_{len(clarify_added) + 1:04d}",
            'class_label': 'CLARIFY',
            'unit_text': text,
            'unit_norm': norm_text(text),
            'unit_type': ku.get('unit_type'),
            'context': ku.get('context'),
            'source_sentence_index': ku.get('source_sentence_index'),
            'source_paragraph_index': ku.get('source_paragraph_index'),
            'safety_level': 'v169_unverified_registry_modifier_swap_no_asserted_answer',
            'clarify_reason': (
                'A same-headword collocate exists in the registry for one word in this '
                'phrase, but it is not verified as a synonym of the original word -- only '
                'that both words pair with the same headword. Not safe to assert as a '
                'correct alternative, so no suggestion is shown.'
            ),
            'reveal_policy': {
                'mode': 'no_suggested_answer',
                'suggestions_role': 'this_class_never_asserts_an_alternative',
            },
            'phase1_prompt': (
                f"This is correct, but could likely be more precise or varied: [{text}]. "
                f"Try rephrasing it yourself -- there is no single suggested answer here."
            ),
            'suggestions': [],
            'evidence_ids': ku.get('evidence_ids', []),
            'candidate_value': ku.get('candidate_value', 0.5),
            'student_facing_task': True,
            'frequency': ku.get('frequency', 1),
        })
    result['keep_units'] = survivors
    result['clarify_units'] = (result.get('clarify_units') or []) + clarify_added
    V165_QA_CACHE['keep_promoted_to_free_enhance'] = 0
    V165_QA_CACHE['unverified_modifier_swaps_routed_to_clarify'] = len(clarify_added)
...
ENGINE_VERSION = "lret-engine-v1.6.9-retire-unsafe-modifier-swap-fix-ctxsyn-merge"
...
def load_detector_output(path: Optional[str]) -> None:
    global DETECTOR_FIX_CANDIDATES, DETECTOR_BROKEN_SENTENCES
...
def _v180_apply_detector_integration(result: Dict[str, Any]) -> None:
...
def _v1110_malformed_sentence_flags(result: Dict[str, Any], essay_text: str = "") -> None:
    ...
    patterns = [
        ...
        (r"\b\w+\s+be\s+\w+\b", "a malformed verb chain around \"be\""),
    ]
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if not args.disable_grammar_flag_check:
        _v1110_malformed_sentence_flags(result, lret_input.get('essay_text') or '')
    result['qa'].setdefault('contract_checks', {})['llm_does_not_create_new_spans'] = True
    result['qa'].setdefault('contract_checks', {})['llm_suggestions_deterministically_validated'] = True
    result['qa'].setdefault('contract_checks', {})['no_embedded_phrase_enhance_bank'] = True
    result['qa'].setdefault('contract_checks', {})['canonical_resources_external_only'] = bool(args.canonical_resources)
