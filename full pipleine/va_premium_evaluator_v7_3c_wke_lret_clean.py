...
ENGINE_ID       = "VA_PREMIUM_EVALUATOR_WKE_V7_3C"
ENGINE_VERSION  = "7.3c.3-lret-payload-cleaner+fix-candidate-normalization"
SCHEMA_VERSION  = "WKM_OUTPUT_V7.3C"
...
def extract_lexical_units(essay_text: str, maps: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    v7.3c: LRET-clean lexical unit extractor.

    Boundary: this extractor does NOT classify units as KEEP/FIX/ENHANCE.
    It only sends LRET complete, replaceable, learning-useful lexical spans.

    Design corrections over v7.3b:
      * no broad all-single-word dump;
      * no all-adjacent-2gram dump;
      * phrase completeness gate before payload export;
      * same-sentence phrase-first dedup;
      * weak prefix fragments suppressed when a fuller span exists;
      * vague placeholder phrases routed as complete CLARIFY-ready spans;
      * formulaic/discourse markers preserved only when complete;
      * extraction QA profile stored on function attribute for consumer payloads.
    """
    sent_map = maps.get('sentence_map') or []
    repeated_counts = {str(r.get('word','')).lower(): int(r.get('count') or 0)
                       for r in ((maps.get('cohesion_map') or {}).get('repeated_content_words') or [])}

    # Universal closed-class / boundary controls. These are not essay-specific.
    boundary_starters = STOPWORDS | {
        'and','but','or','so','because','although','though','while','when','if','unless',
        'which','who','that','there','here','then','now','also','even'
    }
    boundary_enders = STOPWORDS | {
        'and','but','or','so','because','although','though','while','when','if','unless',
        'which','who','that','to','of','for','with','by','from','than','as','is','are','was','were','be','been','being',
        'my','your','his','her','our','their','its'
    }
    aux_modals = {'can','could','may','might','must','should','would','will','shall','is','are','was','were','am','be','been','being','has','have','had','do','does','did'}
    vague = set(VAGUE_NOUNS) | {'kind','kinds','stuff','something','anything','everything'}
    quality_drop_counts: Counter = Counter()
    candidate_source_counts: Counter = Counter()
    ...
    def low_toks(s: str) -> List[str]:
        return [t.lower() for t in toks_of(s)]

    def content_toks(s: str) -> List[str]:
        return [t for t in low_toks(s) if t not in STOPWORDS and len(t) > 2]

    def is_formulaic(text: str) -> bool:
        low = nt(text)
        return low in {c for c in TRANSITION_CUES if ' ' in c}

    def first_last_ok(toks: List[str], *, allow_formulaic: bool = False) -> bool:
        if not toks:
            return False
        low = [t.lower() for t in toks]
        if allow_formulaic and nt(' '.join(toks)) in TRANSITION_CUES:
            return True
        if low[0] in boundary_starters or low[-1] in boundary_enders:
            return False
        return True

    def has_inner_clause_break(toks: List[str]) -> bool:
        if len(toks) <= 2:
            return False
        inner = [t.lower() for t in toks[1:-1]]
        if any(t in {'and','but','or','because','although','though','while','when','if','unless','which','who','that','than','as'} for t in inner):
            return True
        if len(toks) == 3 and inner[0] in {'a','an','the'}:
            return True
        if any(t in {'i','we','you','they','he','she','it','my','your','his','her','our','their','its'} for t in inner):
            return True
        return False

    def looks_like_subject_verb_prefix(text: str) -> bool:
        lt = low_toks(text)
        if len(lt) < 2:
            return False
        # NP + finite/aux/content verb prefix usually needs a complement; LRET should not paraphrase it.
        if lt[1] in aux_modals:
            return True
        if _PREDICATE_VERBS.fullmatch(lt[1]) and lt[0] not in aux_modals:
            return True
        return False

    def looks_like_partial_predicate(text: str, context: str) -> bool:
        lt = low_toks(text)
        if len(lt) <= 1:
            return False
        low = nt(text)
        # verb + adjective but no head noun: "give good", "makes good"
        if len(lt) == 2 and _PREDICATE_VERBS.fullmatch(lt[0]) and lt[1] in {'good','bad','important','easy','hard','strong','stronger','older','young','new','more'}:
            return True
        # phrase is exact prefix of a longer candidate surface in the sentence
        pat = r'\b' + r'\s+'.join(re.escape(t) for t in lt) + r'\s+[A-Za-z]+'
        if re.search(pat, context, flags=re.I) and len(lt) <= 3 and not any(t in vague for t in lt):
            return True
        # dangling prepositional or complement-like tail inside malformed sentence
        if lt[-1] in {'with','for','to','of','about'}:
            return True
        return False

    def phrase_role(text: str) -> str:
        lt = low_toks(text)
        if len(lt) == 1:
            return 'word'
        if is_formulaic(text):
            return 'discourse_or_formulaic_chunk'
        if lt and _PREDICATE_VERBS.fullmatch(lt[0]):
            return 'verb_phrase_or_predicate_chunk'
        if any(_PREDICATE_VERBS.fullmatch(t) for t in lt):
            return 'verb_phrase_or_predicate_chunk'
        return 'noun_phrase'

    def assess_unit(text: str, context: str, source: str) -> Tuple[bool, float, List[str], List[str], str]:
        text = clean_surface(text)
        lt = low_toks(text)
        if len(text) < 3 or not lt:
            return False, 0.0, [], ['empty_or_too_short'], 'empty_or_too_short'
        cts = content_toks(text)
        formulaic = is_formulaic(text)
        if not formulaic and not first_last_ok(lt):
            return False, 0.0, [], ['open_boundary'], 'open_boundary'
        if has_inner_clause_break(lt):
            return False, 0.0, [], ['cross_clause_or_phrase_boundary'], 'cross_clause_or_phrase_boundary'
        if len(lt) > 6:
            return False, 0.0, [], ['overlong_for_lret'], 'overlong_for_lret'
        if not formulaic and len(cts) == 0:
            return False, 0.0, [], ['no_content_tokens'], 'no_content_tokens'
        if looks_like_subject_verb_prefix(text):
            return False, 0.0, [], ['subject_verb_prefix_fragment'], 'subject_verb_prefix_fragment'
        if looks_like_partial_predicate(text, context):
            return False, 0.0, [], ['partial_predicate_or_prefix_span'], 'partial_predicate_or_prefix_span'

        # v7.3c.1 low-value / fragment gates calibrated against LRET v1.4.4 preclassifier.
        # These are universal span-shape/value gates, not essay-specific phrase rules.
        person_heads = {'people','person','persons','children','parents','grandparents','grandchildren','workers','ones'}
        group_modifiers = {'old','older','young','younger','many','some','few','fewer','new'}
        discourse_fragments = {'example','conclusion','hand'}
        temporal_deictics = {'nowadays','today','currently','recently'}
        if not formulaic and any(t in discourse_fragments for t in lt):
            return False, 0.0, [], ['discourse_frame_fragment'], 'discourse_frame_fragment'
        if any(t in temporal_deictics for t in lt) and len(cts) <= 2:
            return False, 0.0, [], ['low_value_temporal_frame'], 'low_value_temporal_frame'
        if 'peoples' in lt:
            return False, 0.0, [], ['malformed_person_plural_not_positive_lret_unit'], 'malformed_person_plural_not_positive_lret_unit'
        if any(t in person_heads for t in lt):
            non_group = [t for t in cts if t not in person_heads and t not in group_modifiers and t not in {'often','still'}]
            # Generic person-group spans are topic mentions, not LRET practice units.
            if len(non_group) == 0 or (len(cts) <= 3 and not any(t in {'population','workforce','employment','labour','labor','care','health','economy','culture','traditions','tradition'} for t in lt)):
                return False, 0.0, [], ['generic_person_group_topic_span'], 'generic_person_group_topic_span'
        if lt and lt[-1].endswith('ing'):
            if any(t in person_heads for t in lt) or len(cts) <= 4:
                return False, 0.0, [], ['dangling_ing_fragment'], 'dangling_ing_fragment'
        if lt and lt[0] in {'need','needs','needed'} and 'more' in lt and len(cts) <= 4:
            return False, 0.0, [], ['quantified_need_vp_not_lret_unit'], 'quantified_need_vp_not_lret_unit'
        if 'everything' in lt and 'for' in lt:
            return False, 0.0, [], ['vague_complex_placeholder_fragment'], 'vague_complex_placeholder_fragment'
        if any(t in {'goverment','government'} for t in lt) and any(t in {'take','make','makes'} for t in lt) and len(cts) <= 3:
            return False, 0.0, [], ['subject_verb_or_malformed_government_fragment'], 'subject_verb_or_malformed_government_fragment'

        axes: List[str] = []
        flags: List[str] = []
        val = 0.50
        if formulaic:
            axes.append('cohesion_expression')
            flags.append('formulaic_discourse_candidate')
            val += 0.12
        if len(cts) >= 2 and not formulaic:
            axes.append('collocation_naturalness')
            flags.append('collocation_candidate')
            val += 0.10
        if any(t in BASIC_ACADEMIC_WORDS for t in lt):
            axes.extend(['topic_vocabulary', 'semantic_specificity'])
            flags.append('topic_relevant')
            val += 0.12
        if any(t in vague for t in lt):
            axes.append('semantic_specificity')
            flags.append('vague_vocabulary_candidate')
            val += 0.10
        if any(t in INFORMAL_PHRASES for t in lt + [nt(text)]):
            axes.append('register_control')
            flags.append('informal_register')
            val += 0.10
        if lt and _PREDICATE_VERBS.fullmatch(lt[0]) and len(cts) >= 2:
            axes.append('predicate_argument')
            flags.append('predicate_argument_candidate')
            val += 0.12
        if re.search(r'\bmore\s+[a-z]+er\b', nt(text)):
            axes.append('semantic_compatibility')
            flags.append('possible_malformed_or_boundary_error')
            val += 0.15
        if source.startswith('detector'):
            flags.append('detector_supported_span')
            val += 0.08
        if not axes:
            axes.append('word_choice')
        val = round(max(0.0, min(1.0, val)), 3)
        if val < 0.50:
            return False, val, sorted(set(axes)), sorted(set(flags)), 'low_candidate_value'
        return True, val, sorted(set(axes)), sorted(set(flags)), 'ok'

    def add_unit(unit_text: str, utype: str, sent: Dict[str, Any], source: str, *, force: bool = False) -> None:
        raw = clean_surface(unit_text)
        if not raw:
            quality_drop_counts['empty_after_cleaning'] += 1
            return
        ok, val, axes, flags, reason = assess_unit(raw, str(sent.get('text') or ''), source)
        if not ok and not force:
            quality_drop_counts[reason] += 1
            return
        lt = low_toks(raw)
        cts = content_toks(raw)
        if len(lt) == 1:
            # v7.3c.1: LRET v1.4.4 treats most isolated words as low-value/noisy.
            # Single-word vocabulary evidence remains inside Evaluator metrics, but is not exported
            # to LRET unless a future detector provides a concrete lexical fix candidate.
            quality_drop_counts['single_word_suppressed_from_lret_payload'] += 1
            return
        key = (nt(raw), int(sent.get('sentence_index', -1)))
        existing = units.get(key)
        if existing:
            existing['frequency'] = int(existing.get('frequency') or 1) + 1
            ev = sent.get('evidence_id')
            if ev and ev not in existing.get('evidence_ids', []):
                existing.setdefault('evidence_ids', []).append(ev)
            existing['candidate_value'] = round(min(1.0, max(float(existing.get('candidate_value') or 0), val) + min(0.06, 0.02 * existing['frequency'])), 3)
            return
        sig = 'high_value_lret_candidate' if val >= 0.70 else 'medium_value_lret_candidate'
        ev = sent.get('evidence_id')
        units[key] = {
            'unit_id': f'lu_{len(units)+1:04d}',
            'unit': raw,
            'unit_text': raw,
            'unit_type': utype or phrase_role(raw),
            ...
            'lret_payload_quality': 'cleaned_v7_3c_complete_span',
        }
        candidate_source_counts[source] += 1

    def add_formulaic_markers(sent: Dict[str, Any]) -> None:
        low = str(sent.get('text') or '').lower()
        for cue in sorted([c for c in TRANSITION_CUES if ' ' in c], key=len, reverse=True):
            if re.search(r'\b' + re.escape(cue) + r'\b', low):
                add_unit(cue, 'discourse_or_formulaic_chunk', sent, 'formulaic_marker_v7_3c')

    def add_quantity_or_vague_phrases(sent: Dict[str, Any]) -> None:
        text = str(sent.get('text') or '')
        patterns = [
            r'\b(?:a\s+lot\s+of|lots\s+of)\s+[A-Za-z][A-Za-z\-]*(?:\s+(?:and\s+)?[A-Za-z][A-Za-z\-]*){0,4}',
            r'\b(?:some|many|several|few|fewer|more)\s+[A-Za-z][A-Za-z\-]*(?:\s+[A-Za-z][A-Za-z\-]*){0,3}',
            r'\b(?:kind|kinds)\s+of\s+[A-Za-z][A-Za-z\-]*',
            r'\b[A-Za-z][A-Za-z\-]*\s+(?:thing|things|stuff|situation|situations|ways)\b',
        ]
        for pat in patterns:
            for m in re.finditer(pat, text, flags=re.I):
                span = clean_surface(re.split(r'\s*,\s*|\s+if\s+|\s+when\s+|\s+which\s+', m.group(0))[0])
                if span:
                    add_unit(span, phrase_role(span), sent, 'vague_or_quantity_phrase_v7_3c')

    def add_content_collocations(sent: Dict[str, Any]) -> None:
        toks = toks_of(str(sent.get('text') or ''))
        if not toks:
            return
        lows = [t.lower() for t in toks]
        # Adjacent content-token bigrams/trigrams only, with strict gates.
        for n in (2, 3):
            for i in range(0, len(toks)-n+1):
                window = toks[i:i+n]
                low = lows[i:i+n]
                if sum(1 for t in low if t not in STOPWORDS and len(t) > 2) < 2:
                    continue
                # Not all content pairs are useful: require academic/repeated/vague/predicate support.
                has_signal = (
                    any(t in BASIC_ACADEMIC_WORDS for t in low)
                    or any(repeated_counts.get(t, 0) >= 2 for t in low if t not in STOPWORDS)
                    or any(t in vague for t in low)
                    or (n >= 2 and _PREDICATE_VERBS.fullmatch(low[0] or ''))
                )
                if not has_signal:
                    quality_drop_counts['collocation_without_signal'] += 1
                    continue
                add_unit(' '.join(window), phrase_role(' '.join(window)), sent, 'content_collocation_v7_3c')

    def add_predicate_phrases(sent: Dict[str, Any]) -> None:
        toks = toks_of(str(sent.get('text') or ''))
        lows = [t.lower() for t in toks]
        for i, w in enumerate(lows):
            if not _PREDICATE_VERBS.fullmatch(w or ''):
                continue
            if w in aux_modals and i + 1 < len(lows) and _PREDICATE_VERBS.fullmatch(lows[i+1] or ''):
                # start from the lexical verb after modal/auxiliary
                continue
            # collect up to four tokens, stopping at clear clause punctuation/conjunction by text window only
            for n in (2, 3, 4):
                if i+n > len(toks):
                    continue
                window = toks[i:i+n]
                low = lows[i:i+n]
                if has_inner_clause_break(window):
                    continue
                if sum(1 for t in low if t not in STOPWORDS and len(t) > 2) < 2:
                    continue
                # Require object/complement signal, not only verb+modifier.
                if n == 2 and low[-1] in {'good','bad','easy','hard','important','older','younger','stronger','new'}:
                    continue
                add_unit(' '.join(window), 'verb_phrase_or_predicate_chunk', sent, 'predicate_phrase_v7_3c')

    # Main extraction pass.
    for sent in sent_map:
        add_formulaic_markers(sent)
        add_quantity_or_vague_phrases(sent)
        add_content_collocations(sent)
        add_predicate_phrases(sent)

    # Single words are intentionally not exported in v7.3c.1; LRET receives complete spans only.

    # Phrase-first suppression: if a shorter unit is contained in a stronger same-sentence unit, keep the stronger unit.
    all_units = list(units.values())
    def rank(u: Dict[str, Any]) -> Tuple[float, int, int]:
        return (float(u.get('candidate_value') or 0.0), len(content_toks(u.get('unit_text') or u.get('unit') or '')), len(toks_of(u.get('unit_text') or u.get('unit') or '')))
    all_units.sort(key=rank, reverse=True)
    survivors: List[Dict[str, Any]] = []
    suppressed: List[Dict[str, Any]] = []
    for cand in all_units:
        ctext = nt(cand.get('unit_text') or cand.get('unit'))
        csent = cand.get('source_sentence_index')
        ctoks = set(content_toks(ctext))
        covered = False
        for s in survivors:
            if s.get('source_sentence_index') != csent:
                continue
            stext = nt(s.get('unit_text') or s.get('unit'))
            stoks = set(content_toks(stext))
            if ctext == stext:
                covered = True
                break
            if ctext and stext and (ctext in stext or (ctoks and ctoks <= stoks and len(stoks) > len(ctoks))):
                # Keep clear vague clarification phrases even if contained, but suppress weak prefixes.
                if 'vague_vocabulary_candidate' in cand.get('extraction_flags', []) and len(content_toks(ctext)) >= 2:
                    continue
                covered = True
                cand['suppressed_by_lret_payload_cleaner'] = s.get('unit_text') or s.get('unit')
                break
        if covered:
    ...
    survivors.sort(key=lambda u: (int(u.get('source_sentence_index') or 0), -float(u.get('candidate_value') or 0.0), str(u.get('unit_text') or u.get('unit')).lower()))
    if len(survivors) > MAX_LEXICAL_UNITS:
        quality_drop_counts['max_lexical_unit_cap_suppressed'] += len(survivors) - MAX_LEXICAL_UNITS
        survivors = sorted(survivors, key=rank, reverse=True)[:MAX_LEXICAL_UNITS]
        survivors.sort(key=lambda u: (int(u.get('source_sentence_index') or 0), str(u.get('unit_text') or u.get('unit')).lower()))

    # Re-number after cleaning.
    for i, u in enumerate(survivors, start=1):
        u['unit_id'] = f'lu_{i:04d}'
        u['unit_text'] = u.get('unit_text') or u.get('unit')
        u['unit'] = u.get('unit') or u.get('unit_text')

    single_count = sum(1 for u in survivors if len(toks_of(u.get('unit_text') or '')) == 1)
    edge_count = sum(1 for u in survivors if 'edge_function_word' in (u.get('extraction_flags') or []))
    lexical_units_quality = {
        'extractor_version': 'v7.3c.3_lret_clean_payload',
        'unit_count': len(survivors),
        'single_word_count': single_count,
        'multiword_count': len(survivors) - single_count,
        'single_word_share': round(single_count / max(1, len(survivors)), 3),
        'edge_function_word_count': edge_count,
        'edge_function_word_rate': round(edge_count / max(1, len(survivors)), 3),
        'suppressed_count': sum(quality_drop_counts.values()) + len(suppressed),
        'suppressed_reason_counts': dict(quality_drop_counts.most_common()),
        'phrase_first_suppressed_count': len(suppressed),
        'source_counts': dict(candidate_source_counts.most_common()),
        'quality_gate_target': {
            'max_unit_count': MAX_LEXICAL_UNITS,
            'max_single_word_share': 0.35,
            'max_edge_function_word_rate': 0.10,
            'target_lret_fragment_noise_rate_after_lret': '<=0.30',
        },
    }
    extract_lexical_units.last_quality_profile = lexical_units_quality
    return survivors
...
def _normalize_lret_fix_family(row: Dict[str, Any]) -> Optional[str]:
    """Map detector families to LRET lexical-only repair families.

    This does not classify LRET units. It only exposes detector-supported lexical
    repair spans when the family is lexical or morphology/word-form based.
    Grammar-only families remain excluded.
    """
    fam = str(row.get('error_family') or row.get('family') or row.get('detector_family') or '').upper().strip()
    span = str(row.get('span_text') or row.get('surface_quote') or row.get('quote') or '').strip()
    low = span.lower()
    lexical_map = {
        'SPELLING': 'SPELLING', 'L_SPELLING': 'SPELLING', 'G_SPELLING': 'SPELLING',
        'WORD_FORM': 'WORD_FORM', 'L_WORD_FORM': 'WORD_FORM', 'MORPHOLOGY': 'WORD_FORM',
        'COLLOCATION': 'COLLOCATION', 'L_COLLOCATION': 'COLLOCATION',
        'WORD_CHOICE': 'WORD_CHOICE', 'L_WORD_CHOICE': 'WORD_CHOICE',
        'LEXICAL_PRECISION': 'LEXICAL_PRECISION', 'L_LEXICAL_PRECISION': 'LEXICAL_PRECISION',
        'SEMANTIC_COMBINATION': 'SEMANTIC_COMBINATION', 'L_SEMANTIC_COMBINATION': 'SEMANTIC_COMBINATION',
        'REGISTER': 'REGISTER', 'L_REGISTER': 'REGISTER',
        'REDUNDANCY': 'REDUNDANCY', 'L_REPETITION': 'REDUNDANCY',
    }
    if fam in lexical_map:
        return lexical_map[fam]
    # Universal double-comparative: lexical word-form repair, even if detector family is grammar-shaped.
    if fam in {'G_COMPARATIVE_FORM', 'COMPARATIVE_FORM', 'COMPARATIVE'} and re.search(r'\bmore\s+[a-z]+er\b', low):
        return 'WORD_FORM_LEXICAL'
    # Preposition pattern is only sent to LRET when explicitly lexical; generic grammar pattern is not.
    if fam in {'PREPOSITION_PATTERN_LEXICAL', 'L_PREPOSITION_PATTERN'}:
        return 'PREPOSITION_PATTERN_LEXICAL'
    return None


def _concrete_lret_fix_suggestion(row: Dict[str, Any]) -> Optional[str]:
    sug = row.get('suggestion') or row.get('suggested_revision')
    if sug is None:
        return None
    s = str(sug).strip()
    if not s:
        return None
    span = str(row.get('span_text') or row.get('surface_quote') or row.get('quote') or '').strip()
    # LRET replacement must be a span replacement, not a full-sentence rewrite.
    if len(words(s)) > max(8, len(words(span)) + 5):
        return None
    if re.search(r'[.!?]', s.strip().rstrip('.')):
        return None
    if span and re.sub(r'[^A-Za-z0-9]+', '', s.lower()) == re.sub(r'[^A-Za-z0-9]+', '', span.lower()):
        return None
    return s


def _build_lret_fix_candidates(detector: Optional[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    rows = detector.get('diagnostic_rows', []) if detector and detector.get('available') else []
    out: List[Dict[str, Any]] = []
    audit = Counter()
    for row in rows:
        span = str(row.get('span_text') or row.get('surface_quote') or row.get('quote') or '').strip()
        if not span:
            audit['missing_span'] += 1
            continue
        mapped = _normalize_lret_fix_family(row)
        if not mapped:
            audit['grammar_or_nonlexical_excluded'] += 1
            continue
        suggestion = _concrete_lret_fix_suggestion(row)
        # Allow deterministic universal double-comparative repair even without suggestion.
        deterministic_possible = mapped == 'WORD_FORM_LEXICAL' and re.search(r'\bmore\s+[a-z]+er\b', span.lower())
        # Allow spelling/word-form if detector has a concrete replacement.
        if not suggestion and not deterministic_possible:
            audit['lexical_candidate_without_concrete_repair_excluded'] += 1
            continue
        out.append({
            'span_text': span,
            'error_family': mapped,
            'detector_family': row.get('error_family') or row.get('family'),
            'suggestion': suggestion,
            'start': row.get('start') if row.get('start') is not None else row.get('span_start'),
            'end': row.get('end') if row.get('end') is not None else row.get('span_end'),
            'paragraph_idx': row.get('paragraph_idx') if row.get('paragraph_idx') is not None else row.get('paragraph_index'),
            'sentence_index': row.get('sentence_index'),
            'context': row.get('local_quote') or row.get('full_sentence') or row.get('sentence') or '',
            'source_detector_row_id': row.get('row_id') or row.get('detector_evidence_id'),
            'lret_fix_mapping_policy': 'v7_3c_lexical_only_normalized_detector_family',
        })
        audit[f'mapped::{mapped}'] += 1
    return out, {
        'source_detector_rows': len(rows),
        'fix_candidate_count': len(out),
        'audit_counts': dict(audit.most_common()),
        'policy': 'lexical-only; grammar-only families excluded; full-sentence suggestions excluded',
    }


def _lret_payload_quality_profile(lexical_units: List[Dict[str, Any]], fix_candidates: List[Dict[str, Any]], fix_audit: Dict[str, Any]) -> Dict[str, Any]:
    unit_count = len(lexical_units or [])
    single = sum(1 for u in lexical_units if len(words(str(u.get('unit_text') or u.get('unit') or ''))) == 1)
    edge = sum(1 for u in lexical_units if 'edge_function_word' in (u.get('extraction_flags') or []))
    profile = getattr(extract_lexical_units, 'last_quality_profile', {}) or {}
    return {
        'extractor_version': 'v7.3c.3_lret_clean_payload',
        'unit_count': unit_count,
        'single_word_count': single,
        'multiword_count': unit_count - single,
        'single_word_share': round(single / max(1, unit_count), 3),
        'edge_function_word_count': edge,
        'edge_function_word_rate': round(edge / max(1, unit_count), 3),
        'fix_candidate_count': len(fix_candidates or []),
        'fix_candidate_audit': fix_audit,
        'extraction_quality_profile': profile,
        'contract': {
            'no_lret_classification_labels': True,
            'complete_span_preferred': True,
            'phrase_first_payload_cleaning': True,
            'lexical_only_fix_candidates': True,
        },
    }
...
def build_consumer_payloads(
    ...
    lret_fix_candidates, lret_fix_audit = _build_lret_fix_candidates(detector)
    lret_quality = _lret_payload_quality_profile(lexical_units, lret_fix_candidates, lret_fix_audit)
    ...
    return {
        ...
        "lret_payload": {
            ...
            "fix_candidates": lret_fix_candidates,
            "payload_quality_profile": lret_quality,
            "note": (
                "No KEEP/FIX/ENHANCE labels; LRET owns lexical classification. "
                "v7.3c pre-cleans spans for LRET by suppressing fragments, weak single words, "
                "open-boundary chunks, and grammar-only fix candidates."
            ),
        },
...
def validate_output(obj: Dict[str, Any], is_ad: bool = False) -> Dict[str, Any]:
    ...
    lret_payload = ((obj.get("consumer_payloads") or {}).get("lret_payload") or {})
    lret_profile = lret_payload.get("payload_quality_profile") or {}
    if lret_profile:
        if int(lret_profile.get("unit_count") or 0) > MAX_LEXICAL_UNITS:
            warnings.append(f"LRET payload exceeds v7.3c unit cap: {lret_profile.get('unit_count')}")
        if float(lret_profile.get("single_word_share") or 0.0) > 0.35:
            warnings.append(f"LRET single-word share high: {lret_profile.get('single_word_share')}")
        if float(lret_profile.get("edge_function_word_rate") or 0.0) > 0.10:
            warnings.append(f"LRET open-boundary/edge rate high: {lret_profile.get('edge_function_word_rate')}")
    else:
        warnings.append("LRET payload_quality_profile missing; v7.3c quality audit unavailable")
...
def evaluate(req: EvaluatorRequest) -> Dict[str, Any]:
    ...
    result = {
        ...
        "boundary_policy": {
            ...
            "lexical_mode_policy":    "v7.3c: Evaluator exports only cleaned, complete, replaceable lexical spans to LRET. Broken/noisy fragments are suppressed or exposed as detector fix candidates only when lexical and concrete.",
        ...
        "lexical_unit_profile": {
            ...
            "policy":                 "v7.3c_cleaned_extraction_only_no_keep_fix_enhance_labels",
            "payload_quality_profile": ((consumer_payloads.get("lret_payload") or {}).get("payload_quality_profile") or {}),
        },
...
if FastAPI is not None:
    app = FastAPI(title="VA Premium Evaluator v7.3c WKE LRET Clean STANDALONE", version=ENGINE_VERSION)

    @app.get("/health")
    def health() -> Dict[str, Any]:
        return {"status":"ok","engine_id":ENGINE_ID,"engine_version":ENGINE_VERSION,
                "schema_version":SCHEMA_VERSION,"boundary":"non-scoring competence-vector evaluator v7.3c with LRET-clean payload"}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="VA Premium Evaluator v7.3c WKE LRET-clean standalone")
    ...
    lp = (result.get("consumer_payloads") or {}).get("lret_payload", {}).get("payload_quality_profile", {})
    print(f"  lret_units={lp.get('unit_count',0)}  single_share={lp.get('single_word_share',0)}  fix_candidates={lp.get('fix_candidate_count',0)}")
